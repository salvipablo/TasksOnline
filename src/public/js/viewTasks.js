...
function showCalendar(year, month) {
  ...
  const calendar = createCalendar(year, month, TASKS)
...
SelViews.addEventListener('change', () => {
  CntTasks.classList.add('no-visible')
  calendarC.classList.add('no-visible')

  if (parseInt(SelViews.value) === 1) CntTasks.classList.remove('no-visible')
  if (parseInt(SelViews.value) === 2) calendarC.classList.remove('no-visible')
})
// Events //
